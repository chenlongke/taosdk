 ...
 * 
 * @author auto create
 * @since 1.0, 2014-01-20 00:00:00
 */
class WeitaoMessageCreateRequest
{
	/** 
	 * act_type=1，创建普通消息，影响用户组在消息中心可以看到消息； act_type=2, 只创建消息，不主动发送，当用户点击菜单并有相应绑定的事件时，才主动来获取消息内容。
	 **/
	private $actType;
	
	/** 
	 * 消息的具体内容，json格式 type=1时，json由text、url属性组成，text必须输入，长度不能超过280字符； type=6时，json由title、imageUrl、text、url组成，每项必须输入，其中title不能超过20字符，imageUrl必须为当前用户图片空间图片，text不能超过60字符； type=7时，json由多组图文属性title、imageUrl、text、url组成，text可以为空但是长度不能超过60字符，其他同type=6规则。 所有url禁止微信的连接地址，所有的内容，含有有非法关键字都不能提交成功。
	 **/
	private $jsonContent;
	...
	/** 
	 * 消息类型，目前只支持：type=1 单条文本、连接消息；type=6 单条图片、标题、文本内容、连接组成的的消息；type=7 由多条图片、标题、文本内容、连接组成的消息。
	 **/
	private $type;
