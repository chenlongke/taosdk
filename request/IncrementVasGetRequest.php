<?php
/**
 * TOP API: taobao.increment.vas.get request
 * 
 * @author auto create
 * @since 1.0, 2011-09-01 00:00:00
 */
class IncrementVasGetRequest
{
	/** 
...
<font color="red">注意：start_modified和end_modified的日期必须在必须在同一天内，比如：start_modified设置2000-01-01 00:00:00，则end_modified必须设置为2000-01-01这个日期</font>
	...
	/** 
	 * 消息所属于的用户的昵称。设置此参数，返回的消息会根据传入nick的进行过滤。自用型appKey的昵称默认为自己的绑定昵称，此参数无效。非自用型appKey必需指定此参数
	 **/
	private $nick;
...
<font color="red">注意：start_modified和end_modified的日期必须在必须在同一天内，比如：start_modified设置2000-01-01 00:00:00，则end_modified必须设置为2000-01-01这个日期</font>
	...
	/** 
	 * 收费消息的所代表的操作状态，默认查询所有状态的数据，除了默认值外每次只能查询一种状态。可选值 VasBillingSuccess（汇金服务后台扣款成功），VasServiceOpen（淘宝某服务，目前是有短信服务会有通知，开通成功）
	 **/
	private $status;
	...
	{
		return "taobao.increment.vas.get";
	}
